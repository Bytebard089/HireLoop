...
```
• Built autonomous recruiting agent using LangGraph (5-node pipeline: JD parsing →
  semantic scoring → adaptive Q-gen → feedback ingestion → XGBoost retraining)

• Designed online XGBoost retraining loop on hiring manager feedback; model adapts
  candidate ranking criteria in real time — feature importances shift visibly after
  every 5 approve/reject decisions

• Implemented dual-modality RAG pipeline (ChromaDB + cross-encoder reranker) over
  resume corpus; semantic similarity score fed as ML feature alongside 3 engineered
  signals (Jaccard skill overlap, experience gap, keyword density)

• Deployed full-stack product on Vercel + Render + Neon; live at [url]
```

---

*Built in 4 weeks as an internship portfolio project. Every component is production-code — no Jupyter notebooks, no hardcoded mock data.*